...
# Set up log file
with open("data/log_de_monolingual_new.txt", "w", encoding="utf-8") as file_out:
...
for d in dataset:
    ...
    s = sent_tokenize(text, language="german")
    ...
    if sent_count >= x:
        ...
        sents_curr = sents[:x]
        with open(f"data/de_monolingual_new/de_monolingual_new_{x-file_lim}_{x}.txt", "w", encoding="utf-8") as file_out:
            for sent in sents_curr:
                token_count += len(sent.split())
                file_out.write(f"{sent}\n")
        # Write entry to log file
        with open("data/log_de_monolingual_new.txt", "a", encoding="utf-8") as file_out:
